...
var grammar = {
    ...
    {"name": "type", "symbols": ["type", "__", (lexer.has("or") ? {type: "or"} : or), "__", "type"], "postprocess": ([t1,,,,t2]) => typeObjects.typeAnd(t1, t2)},
    {"name": "type", "symbols": ["type", "__", (lexer.has("minus") ? {type: "minus"} : minus), "__", "type"], "postprocess": ([t1,,,,t2]) => typeObjects.typeAnd(t1, t2)},
    ...
    {"name": "type", "symbols": [(lexer.has("checkfuns") ? {type: "checkfuns"} : checkfuns)], "postprocess": ([value]) => typeObjects.typeCheckFun(+(value.slice(1)))},
    {"name": "type", "symbols": ["valueType"], "postprocess": ([valueType]) => valueType},
    {"name": "valueType", "symbols": [(lexer.has("true") ? {type: "true"} : true)], "postprocess": id},
    {"name": "valueType", "symbols": [(lexer.has("false") ? {type: "false"} : false)], "postprocess": id},
    {"name": "valueType", "symbols": [(lexer.has("number") ? {type: "number"} : number)], "postprocess": id},
    {"name": "valueType", "symbols": [(lexer.has("string") ? {type: "string"} : string)], "postprocess": id},
    {"name": "type", "symbols": [(lexer.has("_in") ? {type: "_in"} : _in), "__", (lexer.has("lsqBracket") ? {type: "lsqBracket"} : lsqBracket), "params", (lexer.has("rsqBracket") ? {type: "rsqBracket"} : rsqBracket)], "postprocess": ([,,,params,]) => typeObjects.typeIn(params)},
